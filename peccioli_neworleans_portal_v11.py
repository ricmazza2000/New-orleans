...
# Menu navigazione mobile — selectbox nativo
voci_nav = [
    ("🏠", "Home"),
    ("📅", "Briefing"),
    ("🎷", "Temi del viaggio"),
...
@media (max-width: 768px) {
    ...
    .main .block-container { padding-top: 0.3rem !important; padding-bottom: 1rem !important; padding-left: 1rem !important; padding-right: 1rem !important; }
}
/* Nascondi selectbox navigazione su desktop */
.mobile-nav-wrap { display: none; }
@media (max-width: 768px) {
    .mobile-nav-wrap { display: block; margin-bottom: 0.8rem; }
}
</style>
""", unsafe_allow_html=True)

# Selectbox visibile solo su mobile via CSS
st.markdown('<div class="mobile-nav-wrap">', unsafe_allow_html=True)
opzioni = [f"{icon} {label}" for icon, label in voci_nav]
cur_label = f"{next((icon for icon, label in voci_nav if label == active), '🏠')} {active}"
cur_idx = opzioni.index(cur_label) if cur_label in opzioni else 0
scelta = st.selectbox("", opzioni, index=cur_idx, key="mobile_nav", label_visibility="collapsed")
if scelta:
    label_scelta = scelta.split(" ", 1)[1]
    if label_scelta != active:
        st.session_state.nav_target = label_scelta
        st.rerun()
...
st.markdown("<div class='footer-box'>Demo grafica v12 · Portale ragazzi Peccioli × New Orleans 2026</div>", unsafe_allow_html=True)    {"key": "musicale",  "title": "Jazz dal vivo",       "desc": "Gruppo di artisti jazz in una serata nel French Quarter: la musica come anima pulsante della città.",                                   "path": find_image(["home_musicale.jpg"])},
]
...
# CSS per nascondere sidebar e aggiungere padding top su mobile
st.markdown("""
<style>
@media (max-width: 768px) {
    [data-testid="stSidebar"] { display: none !important; }
    ...
    [data-testid="stSidebarCollapsedControl"] { display: none !important; }
    button[kind="header"] { display: none !important; }
    .main .block-container { padding-top: 64px !important; padding-bottom: 1rem !important; padding-left: 1rem !important; padding-right: 1rem !important; }
}
</style>
""", unsafe_allow_html=True)

# Costruisci voci menu
menu_items_html = ""
for icon, label in voci_nav:
    is_active = (label == active)
    bg = "#fff8ee" if is_active else "white"
    ...
    menu_items_html += f"""<a href="?page={page_param}" style="display:flex;align-items:center;gap:0.8rem;padding:0.8rem 1.2rem;text-decoration:none;background:{bg};border-left:3px solid {border};border-bottom:1px solid #f0f0f0;"><span style="font-size:1.1rem;">{icon}</span><span style="font-size:0.92rem;font-weight:{weight};color:{color};font-family:sans-serif;">{label}</span></a>"""
...
<div class="topbar">
    <div class="topbar-title">Peccioli × <span>NOLA</span></div>
