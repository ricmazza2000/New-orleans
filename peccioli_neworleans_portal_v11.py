...
if pagina == "Home":
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .hb-card {{
        ...
        border:1px solid rgba(19,0,137,0.1);box-shadow:0 4px 16px rgba(19,0,137,0.05);
    }}
    .hb-title {{
    ...
    .news-link-text {{
        font-size:0.8rem;font-weight:600;color:{BRAND_BLUE};
    }}
    </style>
...
# ----------------------------
    ...
    briefing_full = [
        {
            ...
            "foto": morelli_img, "emoji": "🏛", "colore": BRAND_YELLOW,
        },
        {
            ...
            "foto": gardner_img, "emoji": "🌐", "colore": BRAND_BLUE,
        },
        {
            ...
            "foto": costa_img, "emoji": "📰", "colore": "#e6b800",
        },
    ]
    ...
    for i, (col, b) in enumerate(zip([c1, c2, c3], briefing_full)):
        b64, mime = img_to_base64(b["foto"])
        foto_tag = f'<img src="data:{mime};base64,{b64}" style="width:90px;height:90px;border-radius:50%;object-fit:cover;border:3px solid {b["colore"]};margin:0 auto 0.8rem;display:block;">' if b64 else f'<div style="width:90px;height:90px;border-radius:50%;background:{b["colore"]};margin:0 auto 0.8rem;display:flex;align-items:center;justify-content:center;font-size:2rem;color:white;">{b["emoji"]}</div>'

        with col:
            st.markdown(f"""
            <div style="background:white;border-radius:22px;padding:1.4rem 1rem 1.2rem;
                 border:1px solid rgba(19,0,137,0.1);box-shadow:0 6px 22px rgba(19,0,137,0.06);
