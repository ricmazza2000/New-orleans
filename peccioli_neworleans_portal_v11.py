...
# Navigazione mobile — cerchietto FAB via components.html
voci_nav = [
    ("🏠", "Home"),
    ("📅", "Briefing"),
    ("🎷", "Temi del viaggio"),
...
for icon, label in voci_nav:
    ...
    border = "3px solid #d08c38" if is_active else "3px solid transparent"
    color = "#d08c38" if is_active else "#14213d"
    weight = "700" if is_active else "500"
    page_param = label.replace(" ", "+")
    fab_items_html += f'<a href="?page={page_param}" style="display:flex;align-items:center;gap:10px;padding:11px 16px;text-decoration:none;background:{bg};border-left:{border};border-bottom:1px solid #f5f5f5;font-size:14px;font-weight:{weight};color:{color};font-family:-apple-system,sans-serif;"><span>{icon}</span><span>{label}</span></a>'
...
<body>
<div class="overlay" id="ov" onclick="close_()"></div>
<div class="fab-menu" id="fm">{fab_items_html}</div>
<div class="fab-btn" id="fb" onclick="toggle()">☰</div>

<script>
function toggle() {{
    var m=document.getElementById('fm');
    var o=document.getElementById('ov');
    var b=document.getElementById('fb');
    if(m.style.display==='block'){{
        m.style.display='none'; o.style.display='none'; b.innerHTML='☰';
    }} else {{
        m.style.display='block'; o.style.display='block'; b.innerHTML='✕';
    }}
}}
function close_() {{
