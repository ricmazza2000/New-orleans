...
gallery_items = [
    ...
    {"key": "sociale",   "title": "Mardi Gras",          "desc": "Un carro del Mardi Gras sfila tra la folla lungo le strade del French Quarter.",                                                        "path": find_image(["Home_carnevale.jpg", "home_carnevale.jpg", "home_sociale.jpg"])},
    {"key": "umana",     "title": "Volti della città",   "desc": "Un ritratto che racconta il lato umano e quotidiano di New Orleans.",                                                                    "path": find_image(["home_umana.jpg"])},
...
@media (max-width: 768px) { .block-container { padding-top: 0.2rem !important; } }
...
.gallery-caption { text-align: center; color: #16324f; font-size: 1rem; margin: 0.5rem 0 1.2rem; font-weight: 500; }
.card { background: white; border-radius: 22px; padding: 1.1rem 1rem; border: 1px solid rgba(20,33,61,0.08); box-shadow: 0 8px 24px rgba(0,0,0,0.06); height: 100%; }
.card-title { font-family: "Playfair Display", Georgia, serif; font-size: 1.05rem; font-weight: 700; color: #14213d; margin-bottom: 0.35rem; }
.note { color: #5b6472; font-size: 0.93rem; line-height: 1.6; }
.legend-card { background: white; border-radius: 18px; padding: 0.95rem; border: 1px solid rgba(20,33,61,0.08); box-shadow: 0 6px 16px rgba(0,0,0,0.05); margin-bottom: 0.75rem; }

@media (max-width: 768px) {
    .block-container { padding-left: 1rem !important; padding-right: 1rem !important; }
}

.footer-box { text-align: center; color: #9aa3b0; font-size: 0.88rem; margin-top: 1.5rem; padding-top: 1rem; border-top: 1px solid rgba(0,0,0,0.07); }
</style>
...
def section_header(numero, sopratitolo, titolo, desc, colore="#d08c38"):
    ...
    <div class="sec-header-full">
        ...
        <div style="position:relative;z-index:1;">
            <div style="font-size:0.68rem;font-weight:700;letter-spacing:0.15em;text-transform:uppercase;color:{colore};margin-bottom:0.35rem;">{sopratitolo}</div>
            <div style="font-family:'Playfair Display',Georgia,serif;font-size:1.8rem;font-weight:800;color:white;line-height:1.1;margin-bottom:0.5rem;">{titolo}</div>
...
# ----------------------------
# SIDEBAR — solo link HTML, non sovrascrive session_state
# ----------------------------
with st.sidebar:
    if logo_path:
...
if pagina == "Home":
    ...
    .hero-full {{
        ...
        padding: 3rem 2rem 2.8rem; text-align: center;
    }}
    @media (min-width: 769px) {{
        .hero-full {{ margin: -1.2rem -3rem 1.6rem -3rem; padding: 3.5rem 3rem 3rem; }}
    }}
    .hero-eyebrow {{ font-size:0.62rem; font-weight:700; letter-spacing:0.2em; text-transform:uppercase; color:rgba(255,255,255,0.4); margin-bottom:0.6rem; }}
    .hero-title {{ font-family:'Playfair Display',Georgia,serif; font-size:clamp(2rem,6vw,3.2rem); font-weight:800; color:white; line-height:1.05; letter-spacing:-0.02em; }}
    .hero-title span {{ color: #d08c38; }}
    .hero-divider {{ width:36px; height:2px; background:#d08c38; border-radius:2px; margin:1rem auto 0; opacity:0.7; }}
    .sticky-topbar {{ position:fixed; top:0; left:0; right:0; height:44px; background:#0d1f3c; display:flex; align-items:center; padding:0 1.2rem; z-index:999998; border-bottom:1px solid rgba(255,255,255,0.08); }}
    .sticky-topbar-title {{ font-size:0.82rem; font-weight:700; color:rgba(255,255,255,0.7); letter-spacing:0.05em; }}
    ...
    header_html = f"""
    <style>
    .sticky-topbar {{ position:fixed; top:0; left:0; right:0; height:44px; background:#0d1f3c; display:flex; align-items:center; justify-content:space-between; padding:0 1.2rem; z-index:999998; border-bottom:1px solid rgba(255,255,255,0.08); }}
    .sticky-topbar-title {{ font-size:0.82rem; font-weight:700; color:rgba(255,255,255,0.7); letter-spacing:0.05em; }}
    .sticky-topbar-title span {{ color: #d08c38; }}
    .main .block-container {{ padding-top: 44px !important; }}
    </style>
...
if pagina == "Home":
    ...
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .quick-btn { display:block; text-align:left; text-decoration:none; background:white; color:#14213d; border-radius:14px; padding:0.85rem 1rem; font-weight:700; font-size:0.88rem; line-height:1.3; border:1px solid rgba(20,33,61,0.09); box-shadow:0 2px 8px rgba(0,0,0,0.04); border-left:3px solid #d08c38; }
    .quick-btn-label { font-size:0.68rem; color:#d08c38; font-weight:700; text-transform:uppercase; letter-spacing:0.08em; display:block; margin-bottom:0.15rem; }
    </style>
    ...
    with st.expander("Scopri a cosa serve →"):
        ...
        Qui troverete il <strong>programma aggiornato</strong>, i <strong>documenti da compilare</strong>,
        i <strong>briefing con gli esperti</strong> e gli <strong>approfondimenti</strong> — libri, film, documentari.
        </p>
        """, unsafe_allow_html=True)
    ...
    .hb-card { background:white;border-radius:20px;padding:1.1rem 1.3rem;border:1px solid rgba(20,33,61,0.08);box-shadow:0 4px 16px rgba(0,0,0,0.04); }
    .hb-title { font-family:'Playfair Display',Georgia,serif;font-size:1rem;font-weight:700;color:#14213d;margin-bottom:0.25rem; }
    .hb-sub { font-size:0.78rem;color:#9aa3b0;margin-bottom:0.7rem; }
    .news-link { display:flex;align-items:center;padding:0.35rem 0;text-decoration:none;border-bottom:1px solid rgba(20,33,61,0.05); }
    ...
    <div class="home-bottom-grid" style="margin-bottom:1rem;">
        <div class="hb-card">
            ...
            <a href="https://www.earthcam.com/usa/louisiana/neworleans/bourbonstreet/" target="_blank"
               style="display:inline-block;background:#0d1f3c;color:white;padding:0.4rem 0.9rem;border-radius:999px;font-size:0.78rem;font-weight:600;text-decoration:none;">
                🎥 Guarda →
            </a>
        </div>
        <div class="hb-card">
...
# ----------------------------
    ...
    briefing_full = [
        {
            "data": "7 maggio", "ora": "ore 21", "titolo": "Elia Morelli",
        ...
        {
            "data": "21 maggio", "ora": "ore 21", "titolo": "Anthony Gardner",
            "ruolo": "Ex ambasciatore USA all'UE · Consiglio di sicurezza nazionale",
            "bio": "Ex ambasciatore degli Stati Uniti presso l'Unione Europea dal 2014 al 2017 su nomina del presidente Obama. Ha lavorato per oltre vent'anni sulle relazioni tra USA ed Europa.",
        ...
        {
            "data": "18 giugno", "ora": "ore 21", "titolo": "Francesco Costa",
            "ruolo": "Giornalista · Direttore de Il Post",
            "bio": "Direttore responsabile de Il Post. Tra i principali divulgatori italiani sulla società e politica americana, autore di libri e progetti dedicati agli Stati Uniti.",
    ...
    for i, (col, b) in enumerate(zip([c1, c2, c3], briefing_full)):
        ...
        with col:
            st.markdown(f"""
            <div style="background:white;border-radius:22px;padding:1.4rem 1rem 1.2rem;border:1px solid rgba(20,33,61,0.08);box-shadow:0 6px 22px rgba(0,0,0,0.06);text-align:center;margin-bottom:0.5rem;">
                {foto_tag}
...
# ----------------------------
    ...
    with tab1:
        ...
        libri = [
            {"titolo": "Una banda di idioti", "autore": "John Kennedy Toole", "anno": "1980 · Pulitzer", "desc": "Capolavoro ambientato nella New Orleans degli anni '60. Satira geniale e irresistibile.", "link": "https://it.wikipedia.org/wiki/Una_banda_di_idioti", "colore": "#d08c38"},
            {"titolo": "Blues Highway", "autore": "Rob Siebert", "anno": "Reportage narrativo", "desc": "Viaggio da Chicago a New Orleans sulle tracce delle origini della musica americana.", "link": "https://marcosymarcos.com/libri/gli-alianti/blues-highway/", "colore": "#17305a"},
            {"titolo": "The Moviegoer", "autore": "Walker Percy", "anno": "1961 · National Book Award", "desc": "Romanzo ambientato a New Orleans, vincitore del National Book Award.", "link": "https://it.wikipedia.org/wiki/Walker_Percy", "colore": "#2e7d5e"},
            {"titolo": "Zeitoun", "autore": "Dave Eggers", "anno": "2009 · Non fiction", "desc": "La storia vera di un siriano-americano rimasto a New Orleans durante Katrina.", "link": "https://it.wikipedia.org/wiki/Zeitoun_(libro)", "colore": "#7b3f00"},
        ]
        for i, libro in enumerate(libri):
            with (col1 if i % 2 == 0 else col2):
                st.markdown(f"""
                <div style="background:white;border-radius:20px;padding:1.2rem 1.3rem;border-top:4px solid {libro['colore']};box-shadow:0 4px 16px rgba(0,0,0,0.06);margin-bottom:1rem;">
                    <div style="font-size:0.7rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:{libro['colore']};margin-bottom:0.3rem;">{libro['anno']}</div>
                    <div style="font-family:'Playfair Display',Georgia,serif;font-size:1.05rem;font-weight:800;color:#14213d;line-height:1.2;margin-bottom:0.15rem;">{libro['titolo']}</div>
                    ...
                    <a href="{libro['link']}" target="_blank" style="font-size:0.78rem;font-weight:600;color:{libro['colore']};text-decoration:none;">Approfondisci →</a>
                </div>
                """, unsafe_allow_html=True)
    ...
    with tab2:
        ...
        for f in film:
            st.markdown(f"""
            <div style="background:white;border-radius:18px;padding:1rem 1.2rem;margin-bottom:0.7rem;display:flex;align-items:center;gap:1rem;border-left:4px solid {f['colore']};box-shadow:0 3px 12px rgba(0,0,0,0.05);">
                <div style="flex:1;">
                ...
                <a href="{f['link']}" target="_blank" style="flex-shrink:0;background:#0d1f3c;color:white;padding:0.35rem 0.8rem;border-radius:999px;font-size:0.75rem;font-weight:600;text-decoration:none;white-space:nowrap;">IMDb →</a>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab3:
        ...
        for d in docs:
            st.markdown(f"""
            <div style="background:white;border-radius:20px;padding:1.3rem 1.4rem;margin-bottom:0.8rem;border-top:4px solid {d['colore']};box-shadow:0 4px 16px rgba(0,0,0,0.06);">
                <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;">
                    ...
                    <a href="{d['link']}" target="_blank" style="flex-shrink:0;background:{d['colore']};color:white;padding:0.4rem 0.9rem;border-radius:999px;font-size:0.78rem;font-weight:600;text-decoration:none;white-space:nowrap;align-self:center;">{d['label']}</a>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab4:
        st.markdown("## ")
        risorse = [
            {"titolo": "New Orleans — Wikipedia italiana", "desc": "Panoramica su storia, cultura, musica e geografia.", "link": "https://it.wikipedia.org/wiki/New_Orleans", "colore": "#17305a"},
            {"titolo": "The Times-Picayune · NOLA.com", "desc": "Il principale quotidiano di New Orleans.", "link": "https://www.nola.com", "colore": "#d08c38"},
        ...
        for r in risorse:
            ...
            <div style="background:white;border-radius:18px;padding:1.1rem 1.3rem;margin-bottom:0.7rem;display:flex;align-items:center;gap:1rem;border:1px solid rgba(20,33,61,0.08);box-shadow:0 3px 12px rgba(0,0,0,0.05);">
...
# ----------------------------
    ...
    svg_musica = '<svg viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;opacity:0.12;position:absolute;bottom:0;right:0;"><circle cx="160" cy="60" r="50" fill="#d08c38"/><path d="M20 60 Q50 30 80 60 Q110 90 140 60" stroke="#d08c38" stroke-width="4" fill="none"/><circle cx="170" cy="35" r="8" fill="#d08c38"/><line x1="178" y1="35" x2="178" y2="5" stroke="#d08c38" stroke-width="3"/></svg>'
    svg_resilienza = '<svg viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;opacity:0.12;position:absolute;bottom:0;right:0;"><path d="M0 90 Q25 70 50 90 Q75 110 100 90 Q125 70 150 90 Q175 110 200 90 L200 120 L0 120 Z" fill="#17305a"/><polygon points="100,15 115,55 85,55" fill="#d08c38"/><rect x="92" y="55" width="16" height="30" fill="#d08c38"/></svg>'
    svg_societa = '<svg viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;opacity:0.12;position:absolute;bottom:0;right:0;"><circle cx="50" cy="40" r="14" fill="#2e7d5e"/><rect x="38" y="54" width="24" height="35" rx="4" fill="#2e7d5e"/><circle cx="100" cy="35" r="16" fill="#d08c38"/><rect x="86" y="51" width="28" height="40" rx="4" fill="#d08c38"/><circle cx="150" cy="40" r="14" fill="#17305a"/><rect x="138" y="54" width="24" height="35" rx="4" fill="#17305a"/></svg>'
    svg_storia = '<svg viewBox="0 0 200 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%;opacity:0.12;position:absolute;bottom:0;right:0;"><path d="M60 110 L60 50 Q60 20 90 20 Q120 20 120 50 L120 110" stroke="#7b3f00" stroke-width="5" fill="none"/><line x1="45" y1="65" x2="135" y2="65" stroke="#7b3f00" stroke-width="4"/><line x1="43" y1="80" x2="137" y2="80" stroke="#7b3f00" stroke-width="4"/><circle cx="155" cy="30" r="5" fill="#d08c38"/><circle cx="170" cy="50" r="4" fill="#d08c38"/></svg>'

    temi = [
        {"titolo": "Musica", "label": "JAZZ", "colore": "#d08c38", "bg": "#fff8ee", "sottotitolo": "Jazz, blues e il ritmo della città", "domanda": "Perché il jazz è nato proprio qui, e non altrove?", "desc": "New Orleans è la culla del jazz e del blues. La musica non è intrattenimento: è linguaggio sociale, memoria collettiva, forma di resistenza. Dalle second line nei funerali di strada a Frenchmen Street la sera, la città vive attraverso il suono.", "luoghi": "Frenchmen Street · Congo Square · Louis Armstrong Park", "svg": svg_musica},
        {"titolo": "Resilienza", "label": "KATRINA", "colore": "#17305a", "bg": "#f0f4fb", "sottotitolo": "Katrina, ricostruzione e cambiamento climatico", "domanda": "Come si ricostruisce una città dopo che l'acqua se la porta via?", "desc": "Il 2005 ha messo a nudo le fragilità strutturali della città: infrastrutture, disuguaglianze razziali, risposta istituzionale. Vent'anni dopo, la città è ancora in cammino.", "luoghi": "Lower Ninth Ward · Argini del Mississippi · Lakeview", "svg": svg_resilienza},
        {"titolo": "Società", "label": "PEOPLE", "colore": "#2e7d5e", "bg": "#f0faf5", "sottotitolo": "Diversità culturale, questioni razziali, umanità", "domanda": "Cosa succede quando culture lontanissime vivono nello stesso isolato da tre secoli?", "desc": "New Orleans è una delle città più multiculturali e diseguali degli Stati Uniti. L'eredità della schiavitù, la comunità creola, le contraddizioni tra turismo e vita reale.", "luoghi": "Congo Square · Tremé · Garden District", "svg": svg_societa},
        {"titolo": "Identità e storia", "label": "NOLA", "colore": "#7b3f00", "bg": "#fdf6f0", "sottotitolo": "Radici coloniali, voodoo, Mardi Gras, French Quarter", "domanda": "Perché New Orleans non somiglia a nessun'altra città americana?", "desc": "Fondata dai francesi, ceduta agli spagnoli, acquistata dagli Stati Uniti. Questa stratificazione di culture ha prodotto un'identità unica: il French Quarter, il voodoo, il Mardi Gras.", "luoghi": "French Quarter · St. Louis Cemetery · Jackson Square", "svg": svg_storia},
    ]
    ...
    for i, tema in enumerate(temi):
        with (col1 if i % 2 == 0 else col2):
            with st.expander(f"{tema['titolo']}  ·  {tema['sottotitolo']}", expanded=True):
                st.markdown(f"""
                <div style="position:relative;background:{tema['bg']};border-radius:16px;padding:1.2rem 1rem 1rem;border:1px solid {tema['colore']}22;overflow:hidden;">
                    {tema['svg']}
                    <div style="position:absolute;top:-10px;right:12px;font-family:'Playfair Display',Georgia,serif;font-size:4.5rem;font-weight:800;color:{tema['colore']};opacity:0.07;line-height:1;user-select:none;pointer-events:none;">{tema['label']}</div>
                    <div style="position:relative;z-index:1;">
                        <div style="font-size:0.92rem;font-style:italic;color:{tema['colore']};font-weight:600;margin-bottom:0.7rem;line-height:1.5;">&ldquo;{tema['domanda']}&rdquo;</div>
                        <div style="font-size:0.87rem;color:#3a4a5c;line-height:1.65;margin-bottom:0.8rem;">{tema['desc']}</div>
                        <div style="font-size:0.75rem;font-weight:600;color:{tema['colore']};border-top:1px solid {tema['colore']}30;padding-top:0.6rem;">&#128205; {tema['luoghi']}</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
# ----------------------------
    ...
    def mostra_mappa():
        ...
        for luogo in luoghi_dati:
            folium.CircleMarker(
                ...
                popup=folium.Popup(f"<b style='font-size:14px'>{luogo['nome']}</b><br><span style='font-size:12px;color:#444'>{luogo['desc']}</span>", max_width=250),
    ...
    <div style="display:flex;flex-wrap:wrap;gap:0.6rem;margin-bottom:1.2rem;">
        <div style="display:flex;align-items:center;gap:0.4rem;background:white;padding:0.35rem 0.8rem;border-radius:999px;border:1px solid rgba(0,0,0,0.08);font-size:0.82rem;font-weight:600;"><div style="width:12px;height:12px;border-radius:50%;background:#c0392b;flex-shrink:0;"></div> Identità e storia</div>
        <div style="display:flex;align-items:center;gap:0.4rem;background:white;padding:0.35rem 0.8rem;border-radius:999px;border:1px solid rgba(0,0,0,0.08);font-size:0.82rem;font-weight:600;"><div style="width:12px;height:12px;border-radius:50%;background:#d4a017;flex-shrink:0;"></div> Musica</div>
        <div style="display:flex;align-items:center;gap:0.4rem;background:white;padding:0.35rem 0.8rem;border-radius:999px;border:1px solid rgba(0,0,0,0.08);font-size:0.82rem;font-weight:600;"><div style="width:12px;height:12px;border-radius:50%;background:#17305a;flex-shrink:0;"></div> Resilienza</div>
        <div style="display:flex;align-items:center;gap:0.4rem;background:white;padding:0.35rem 0.8rem;border-radius:999px;border:1px solid rgba(0,0,0,0.08);font-size:0.82rem;font-weight:600;"><div style="width:12px;height:12px;border-radius:50%;background:#2e7d5e;flex-shrink:0;"></div> Società</div>
    </div>
    """, unsafe_allow_html=True)
...
# ----------------------------
    ...
    <div style="background:#fff8ee;border:2px dashed #d08c38;border-radius:24px;padding:2.5rem 2rem;text-align:center;max-width:580px;margin:2rem auto;">
        ...
        <div style="font-size:0.97rem;color:#5b6472;line-height:1.75;">Il programma dettagliato del viaggio è ancora in fase di costruzione.<br>Questa sezione verrà aggiornata con tutte le tappe, gli appuntamenti e le attività non appena il percorso sarà definito.</div>
    </div>
    """, unsafe_allow_html=True)
...
# ----------------------------
    ...
    st.markdown("""
    <div style="background:#f0f4fb;border-left:4px solid #17305a;border-radius:0 12px 12px 0;padding:0.8rem 1.2rem;margin-bottom:1.4rem;font-size:0.88rem;color:#17305a;">
    ...
    for icona, titolo, desc, completato in documenti:
        ...
        st.markdown(f"""
        <div style="background:white;border-radius:14px;padding:0.9rem 1.1rem;margin-bottom:0.6rem;display:flex;align-items:center;gap:1rem;border:1px solid rgba(20,33,61,0.07);box-shadow:0 2px 8px rgba(0,0,0,0.04);">
            ...
            <div style="flex-shrink:0;"><div style="font-size:0.72rem;font-weight:600;color:{colore_stato};">{stato_testo}</div></div>
        </div>
        """, unsafe_allow_html=True)

st.markdown("<div class='footer-box'>Portale ragazzi Peccioli × New Orleans 2026</div>", unsafe_allow_html=True)    {"key": "musicale",  "title": "Jazz dal vivo",       "desc": "Gruppo di artisti jazz in una serata nel French Quarter: la musica come anima pulsante della città.",                                   "path": find_image(["home_musicale.jpg"])},
]
...
st.markdown("<div class='footer-box'>Demo grafica v12 · Portale ragazzi Peccioli × New Orleans 2026</div>", unsafe_allow_html=True)    {"key": "umana",     "title": "Volti della città",   "desc": "Un ritratto che racconta il lato umano e quotidiano di New Orleans, attraverso le persone che la vivono.",                               "path": find_image(["home_umana.jpg"])},
