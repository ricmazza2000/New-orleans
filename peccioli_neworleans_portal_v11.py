...
if pagina == "Home":
    nola_tag_header = f'<img src="data:{nola_logo_mime};base64,{nola_logo_b64}" style="height:36px;opacity:0.7;filter:brightness(0) saturate(100%) invert(16%) sepia(60%) saturate(500%) hue-rotate(190deg);">' if nola_logo_b64 else ""

    header_html = f"""
    <style>
    .h-desktop {{ display: flex; }}
    .h-mobile  {{ display: none; }}
    @media (max-width: 600px) {{
        .h-desktop {{ display: none !important; }}
        .h-mobile  {{ display: flex !important; }}
        .header-box {{ padding: 0.9rem 1rem !important; }}
    }}
    </style>
    <div class="header-box" style="border-radius:20px;background:linear-gradient(135deg,#f8f5f0 0%,#fdf9f4 100%);border:1px solid rgba(208,140,56,0.15);padding:1.4rem 1.5rem 1.2rem;margin-bottom:1.2rem;">
        <!-- DESKTOP -->
        <div class="h-desktop" style="align-items:center;gap:1.2rem;">
            {logo_tag}
        ...
        <!-- MOBILE -->
        <div class="h-mobile" style="flex-direction:column;gap:0.6rem;">
            <div style="display:flex;align-items:center;justify-content:space-between;">
                {logo_tag}
                {nola_tag_header}
            </div>
            <div style="font-family:'Playfair Display',Georgia,serif;font-size:1.6rem;font-weight:800;color:#0d1f3c;line-height:1.1;letter-spacing:-0.01em;">
                Peccioli &times; <span style="color:#d08c38;">New Orleans</span> 2026
            </div>
        </div>
    </div>
